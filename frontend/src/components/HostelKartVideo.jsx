...
export const HostelKartVideo = ({ 
  ...
  poster,
  title = 'Experience HostelKart',
  subtitle = 'HostelKart brings hostel essentials, shopping, delivery and AI assistance together.'
}) => {
  return (
    <section 
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1 select-none">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-1.5 bg-emerald-100 text-emerald-700 rounded-xl flex items-center justify-center">
        ...
        <div className="hidden sm:flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
          ...
          <span>•</span>
          <span className="flex items-center gap-1"><Sparkles className="w-3 h-3 text-emerald-500" /> AI Assistant</span>
          <span>•</span>
          <span className="flex items-center gap-1"><ShieldCheck className="w-3 h-3 text-blue-500" /> Room Essentials</span>
        </div>
      </div>
      ...
      <motion.div 
        ...
        className="relative overflow-hidden bg-slate-950 rounded-2.5xl sm:rounded-3xl shadow-premium border border-slate-200/80 group"
      >
        <div className="aspect-video w-full bg-slate-950 flex items-center justify-center relative">
          <video
            className="w-full h-full object-contain rounded-2.5xl sm:rounded-3xl"
            controls
            muted
            playsInline
            preload="metadata"
            poster={poster}
            aria-label="Experience HostelKart promotional demonstration video"
          >
